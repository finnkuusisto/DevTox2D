...
def l2logreg_predict_for_one(train_comps, test_comp, train_exdict, test_exdict,fold_keep_inds, params, pos_lab, neg_lab):
    ...
    # do per-fold normalization ([0,1] scaling flavor)
    ss = StandardScaler()
    ss.fit(x_train)
    x_train = ss.transform(x_train)
    x_test = ss.transform(x_test)
    # train a model
    est = LogisticRegression(dual=True)
    clf = GridSearchCV(est, param_grid=params, iid=False)
...
for test_comp in uniq_comps:
    ...
    day_preds = l2logreg_predict_for_one(train_comps, test_comp, train_exdict, test_exdict, fold_keep_inds, PARAMS, POS_LAB, NEG_LAB)
